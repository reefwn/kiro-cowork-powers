...
Read `TASKS.md` and `memory/` directory. If they don't exist, suggest running the start workflow first.

### 2. Sync Tasks from External Sources

Check for available task sources:
- **~~project tracker** (if MCP available)
...
- **~~knowledge base:** List recently touched docs
